...
/**
 * IRS 평가 이력 레포지토리 — Append-Only 이력 관리.
 *
 * @see K-IFRS 1109호 6.5.8 (공정가치 위험회피 — 이력 보존)
 * @see K-IFRS 1107호 (금융상품 공시 — 평가 이력 보존 의무)
 */
public interface IrsValuationRepository extends JpaRepository<IrsValuation, Long> {

    /**
     * 계약별 평가 이력 조회 (생성일시 내림차순 — 최신순).
     */
    Page<IrsValuation> findByContractIdOrderByCreatedAtDesc(String contractId, Pageable pageable);
     ...
     */
    @Query("SELECT v FROM IrsValuation v WHERE v.contractId = :contractId " +
           "AND v.valuationDate < :valuationDate " +
           "ORDER BY v.createdAt DESC LIMIT 1")
    Optional<IrsValuation> findLatestBeforeDate(
